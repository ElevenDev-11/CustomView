...
public class OverFlyingLayoutManager extends RecyclerView.LayoutManager
     ...
     */
    protected float setViewElevation(@NonNull View itemView, float targetOffset) {
    ...
    /**
     * 可以在此处设置自己的属性或更改现有属性，例如 mSpaceMain 和 mSpaceInOther
     */
    ...
    private void layoutScrap(View scrap, float targetOffset) {
        ...
        if (mOrientation == VERTICAL) {
            layoutDecorated(scrap, mSpaceInOther + left, mSpaceMain + top,
                    mSpaceInOther + left + mDecoratedMeasurementInOther, mSpaceMain
                            + top + mDecoratedMeasurement);
        } else {
            layoutDecorated(scrap, mSpaceMain + left, mSpaceInOther + top,
                    mSpaceMain + left + mDecoratedMeasurement, mSpaceInOther
                            + top + mDecoratedMeasurementInOther);
        }

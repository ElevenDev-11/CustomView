...
import cn.elevendev.bottomtablayout.util.SizeUtil;
...
public class TabButton extends View {
    ...
    private int mOffset = 5; // 偏移量
    private String mMenuColor = "#ffffff";  // Menu背景色
    ...
    private void init() {
        mTextSize = (int) SizeUtil.Dp2Px(getContext(), 12);
        ...
        mText = "按钮";
        mDrawable = ResourcesCompat.getDrawable(getResources(), R.mipmap.ic_launcher, getContext().getTheme());
        mMaxHeight = (int) SizeUtil.Dp2Px(getContext(), 60);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        canvas.drawColor(Color.parseColor(mMenuColor));
        ...
        int offset = (int) SizeUtil.Dp2Px(getContext(), mOffset);
        int r = (mIconHeight - offset) / 2;
        mDrawable.setBounds(-r, -r + offset, r, r + offset);
        ...
        //绘制未读消息
        if (mNews != -1) {
            canvas.translate(getWidth() / 2 + r, mIconHeight / 2);
